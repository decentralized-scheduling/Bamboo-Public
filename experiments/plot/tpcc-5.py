...
with open("tpcc-5.out", "r") as f:
    ...
    while True:
        ...
        throughputs[alg][0].append(nr_wh)
        arates[alg][0].append(nr_wh)
        latencies[alg][0].append(nr_wh)
...
for i in range(len(throughputs["QCC"][1])):
    for c in combine:
...
for alg in algs:
    if alg == "TICTOC" or alg == "WAIT_DIE":
        continue
    ax[0].plot(throughputs[alg][1], label=algs[alg][0], zorder=3, linewidth=1.25, marker=algs[alg][2], color=algs[alg][1], markersize=6, linestyle=algs[alg][3])
    ax[1].plot(latencies[alg][1], label=algs[alg][0], zorder=3, linewidth=1.25, marker=algs[alg][2], color=algs[alg][1], markersize=6, linestyle=algs[alg][3])
    ax[2].plot(arates[alg][1], label=algs[alg][0], zorder=3, linewidth=1.25, marker=algs[alg][2], color=algs[alg][1], markersize=6, linestyle=algs[alg][3])

xticklabels = [str(t) for t in arates["QCC"][0]]

for i in range(nplots):
    ax[i].grid(True, axis="both", linestyle="--", lw=0.5, color="#555555", zorder=0)

    ax[i].set_xticks([x for x in range(len(xticklabels))])
    ax[i].set_xticklabels(xticklabels)
    ax[i].tick_params(labelsize=ticksize)
    ax[i].set_xlabel("# of warehouses", fontsize=fontsize)
...
fig.savefig("tpcc-5.pdf", bbox_inches="tight", pad_inches=0.03, format='pdf')
